...
{
	var size = mxUtils.getValue(state.style, mxConstants.STYLE_FONTSIZE,
		mxConstants.DEFAULT_FONTSIZE);
	var family = mxUtils.getValue(state.style, mxConstants.STYLE_FONTFAMILY,
	...
	var parentStyle = this.graph.getCellStyle(this.graph.model.getParent(state.cell), true);

	if (size == 'inherit')
	{
		size = (parentStyle != null) ? mxUtils.getValue(parentStyle, mxConstants.STYLE_FONTSIZE,
			mxConstants.DEFAULT_FONTSIZE) : mxConstants.DEFAULT_FONTSIZE;
	}
	
	if (family == 'inherit')
	{
		family = (parentStyle != null) ? mxUtils.getValue(parentStyle, mxConstants.STYLE_FONTFAMILY,
			mxConstants.DEFAULT_FONTFAMILY) : mxConstants.DEFAULT_FONTFAMILY;
	}
	...
	var color = mxUtils.getValue(state.style, mxConstants.STYLE_FONTCOLOR);

	if (color == null || color == mxConstants.NONE)
	{
		color = this.graph.shapeForegroundColor;
	}

	this.textarea.style.color = mxUtils.getLightDarkColor(color).cssText;
	var backgroundColor = this.getBackgroundColor(state);

	if (backgroundColor != null)
	{
		this.textarea.style.backgroundColor = mxUtils.getLightDarkColor(backgroundColor).cssText;
	}
	else
	{
	...
	if (borderColor != null)
	{
		this.textarea.style.border = '1px solid ' + mxUtils.getLightDarkColor(borderColor).cssText;
	}
	else
	{
	...
	if (dir == mxConstants.TEXT_DIRECTION_AUTO && state != null && state.text != null &&
		state.text.dialect != mxConstants.DIALECT_STRICTHTML && !mxUtils.isNode(state.text.value))
	{
 ...
 * 
 * Sets the given value as the editing value.
 */
...
mxCellEditor.prototype.startEditing = function(cell, trigger, initialText)
{
	...
	if (state != null)
	{
		...
		this.setEditingValue(state, this.getInitialValue(state, trigger) || '');
