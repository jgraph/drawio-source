 ...
 * prefixed hex color code (eg. "#FFEA80"), or "fill", "stroke" or "font" to
 ...
 */
mxStencil.prototype.drawShape = function(canvas, shape, x, y, w, h, graph)
{
	graph = (graph != null) ? graph : ((shape != null &&
		shape.state != null) ? shape.state.view.graph : null);
	...
	this.drawChildren(canvas, shape, x, y, w, h, this.bgNode, aspect, false, true, graph);
	this.drawChildren(canvas, shape, x, y, w, h, this.fgNode, aspect, true,
		!shape.outline || shape.style == null || mxUtils.getValue(
		shape.style, mxConstants.STYLE_BACKGROUND_OUTLINE, 0) == 0, graph);
 ...
 * Draws this stencil inside the given bounds.
 */
mxStencil.prototype.drawChildren = function(canvas, shape, x, y, w, h, node, aspect, disableShadow, paint, graph)
{
	if (node != null && w > 0 && h > 0)
	{
		...
		while (tmp != null)
		{
			...
			{
				this.drawNode(canvas, shape, tmp, aspect, disableShadow, paint, graph);
			}
 ...
 * Draws this stencil inside the given bounds.
 */
mxStencil.prototype.drawNode = function(canvas, shape, node, aspect, disableShadow, paint, graph)
{
	...
	else if (paint)
	{
		if (name == 'path')
		{
			...
			if (parseRegularly)
			{
				...
				while (childNode != null)
				{
					if (childNode.nodeType == mxConstants.NODETYPE_ELEMENT)
					{
						this.drawNode(canvas, shape, childNode, aspect, disableShadow, paint, graph);
					}
		...
		else if (name == 'include-shape')
		{
			...
			if (stencil != null)
			{
				...
				stencil.drawShape(canvas, shape, x, y, w, h, graph);
			}
		}
		...
		{
			canvas.setStrokeColor(this.parseColor(canvas, shape,
				node, node.getAttribute('color'), graph));
		}
		...
		{
			canvas.setFillColor(this.parseColor(canvas, shape,
				node, node.getAttribute('color'), graph));
		}
		else if (name == 'alpha')
		{
		...
		{
			canvas.setFontColor(this.parseColor(canvas, shape,
				node, node.getAttribute('color'), graph));
		}
